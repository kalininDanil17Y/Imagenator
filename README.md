...
Позволяет создать простенький сайт, не используя сложные фреймворки как Laravel, Imagenator это мини-фраймворк который поддерживает роутинг (POST/GET), и есть встроеный шаблонизатор
...
Загружает .env и другое

---
### 1.2 /app
Содержит классы framework'ка

---
### 1.3 Создание роута
В файле **/app/App.php** указываются роуты
...
---
### 1.4 /src
Папка с дополнительными файлами, контроллерами, моделями, и миграциями

---
### 1.5 Создание конроллера
...
{
    return $response->view('form') // Выбрать шаблон form
        ->setStatus(201) //Указать что код ответа 201
        ->setHeader('Content-type', 'text/html;') //Указать header
        ->setBody('sample text') //Выводит на экран текст (отменяет действие view)
        ->redirect('location', 'code (optional)'); //создаёт редирект
    /**
